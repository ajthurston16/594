...
public class MyBoard implements Board {
	...
	public boolean placePiece(int row, char column, boolean isWhitesTurn) {
	    ...
	    if (this.isMoveLegal(row, column, isWhitesTurn)) {
	        ...
	        /* If a piece was placed successfully, then flip squares and switch turns (need to do this
	           now or the board won't print the correct turn) */
	        this.flipSquares();
	        this.isWhitesTurn = !isWhitesTurn;
	        this.printBoard();
	        return true;
	    }
	    this.squaresToFlip.clear(); //Clear the stack of square, just to be safe
	    return false;
	}
